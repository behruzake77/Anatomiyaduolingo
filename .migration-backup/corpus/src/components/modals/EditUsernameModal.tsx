"use client";

import { useState } from "react";
import { X } from "lucide-react";
import { useAppStore } from "@/store/useAppStore";
import { updateUsername } from "@/lib/auth";
import { useStrings } from "@/i18n";

interface EditUsernameModalProps {
  onClose: () => void;
}

export function EditUsernameModal({ onClose }: EditUsernameModalProps) {
  const currentUser = useAppStore((s) => s.currentUser);
  const [newUsername, setNewUsername] = useState(currentUser?.username || "");
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);
  const t = useStrings();

  const handleSave = async () => {
    if (!newUsername.trim()) {
      setError("Foydalanuvchi nomi bo'sh bo'lishi mumkin emas");
      return;
    }

    if (newUsername === currentUser?.username) {
      setError("Yangi foydalanuvchi nomi bilan bir xil");
      return;
    }

    setLoading(true);
    const result = await updateUsername(newUsername);
    setLoading(false);

    if (result.success) {
      onClose();
    } else {
      setError(result.error || "Xatolik yuz berdi");
    }
  };

  return (
    <div className="fixed inset-0 z-50 flex items-end bg-black/50">
      <div className="w-full animate-in slide-in-from-bottom-5 rounded-t-2xl bg-surface p-5">
        <div className="mb-6 flex items-center justify-between">
          <h2 className="text-lg font-bold">Foydalanuvchi nomini o'zgartirish</h2>
          <button
            onClick={onClose}
            className="rounded-lg p-2 hover:bg-surface2"
            aria-label="Yopish"
          >
            <X className="h-5 w-5" />
          </button>
        </div>

        <input
          type="text"
          value={newUsername}
          onChange={(e) => {
            setNewUsername(e.target.value);
            setError("");
          }}
          placeholder="Yangi foydalanuvchi nomi"
          className="w-full rounded-xl border border-line bg-surface2 px-4 py-3 mb-4"
          maxLength={32}
        />

        {error && <div className="mb-4 text-sm text-danger">{error}</div>}

        <div className="flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 rounded-xl border border-line px-4 py-3 font-medium transition hover:bg-surface2"
          >
            Bekor qilish
          </button>
          <button
            onClick={handleSave}
            disabled={loading}
            className="flex-1 rounded-xl bg-primary px-4 py-3 font-medium text-white transition hover:bg-primary/90 disabled:opacity-50"
          >
            {loading ? "Saqlanmoqda..." : "Saqlash"}
          </button>
        </div>
      </div>
    </div>
  );
}
